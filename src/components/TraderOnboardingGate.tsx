...
interface TraderOnboardingGateProps {
  language: LanguageCode;
  initialData?: TraderIdentity | null;
  onSaveAndContinue: (identity: TraderIdentity) => void;
}

export const TraderOnboardingGate: React.FC<TraderOnboardingGateProps> = ({
  language,
  initialData,
  onSaveAndContinue,
}) => {
  ...
  const [tradingFirm, setTradingFirm] = useState<string>(initialData?.tradingFirm || 'Sri Venkateswara Agro Traders');
  const [district, setDistrict] = useState<string>(initialData?.district || 'Guntur');
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const identity: TraderIdentity = {
      ...
      tradingFirm: tradingFirm.trim() || 'AP Agro Traders',
  ...
  return (
    <div className="w-full max-w-xl mx-auto py-6 sm:py-10 animate-in fade-in zoom-in-95 duration-200">
      <div className="bg-white rounded-3xl border-2 border-[#0d5c2e]/20 shadow-2xl overflow-hidden">
        {/* Top Gradient Banner */}
        <div className="bg-gradient-to-r from-[#0d5c2e] via-[#14532d] to-[#1e3a5f] text-white p-6 sm:p-8 text-center relative overflow-hidden">
          ...
          <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white">
            {language === 'te' ? 'ట్రేడర్ సైన్ అప్ / నమోదు' : 'Trader Registration & Sign Up'}
          </h2>
          <p className="text-xs sm:text-sm text-emerald-100 max-w-md mx-auto mt-1.5 leading-relaxed">
            {language === 'te'
              ? 'ట్రేడర్ డాష్‌బోర్డ్‌లోకి ప్రవేశించడానికి దయచేసి మీ పేరు మరియు ఫోన్ నంబర్‌ను నమోదు చేసి సైన్ అప్ చేయండి.'
              : 'Please enter your details to register and access direct farmer listings and procurement.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="p-6 sm:p-8 space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs sm:text-sm font-bold text-[#0d5c2e] uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span>{language === 'te' ? '1. మీ పూర్తి పేరు (Full Name) *' : '1. Enter Your Full Name *'}</span>
            </label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase mb-1">
                {language === 'te' ? 'సంస్థ పేరు (Trading Firm)' : 'Trading Firm / Agency'}
              </label>
              <input
                type="text"
                value={tradingFirm}
                onChange={(e) => setTradingFirm(e.target.value)}
                placeholder="e.g. Sri Venkateswara Agro"
          ...
          <div className="pt-3">
            <button
              type="submit"
              className="w-full h-13 bg-gradient-to-r from-[#0d5c2e] to-[#14532d] hover:from-[#14532d] hover:to-[#042f2e] text-white rounded-2xl text-sm font-black tracking-wide flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-xl active:scale-[0.99] cursor-pointer"
            >
              <span className="material-symbols-outlined text-[20px]">how_to_reg</span>
              <span>{language === 'te' ? 'నమోదు చేసి డాష్‌బోర్డ్‌కి వెళ్లండి' : 'Register & Open Trader Dashboard'}</span>
